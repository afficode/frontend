...
import { dateManiputator, getToken } from "../../../utils/index";
...
const Service = ({ data }) => {
  ...
  const grapProduct = async () => {
    axiosWithToken
      .get(`api/grabProduct?userId=${userId}&productId=${productId}`)
      .then(({ data }) => {});
  };
          ...
          <div id="middleTop" className="w-full">
            ...
            >
              ...
              <div className="flex items-center justify-center flex-wrap gap-4 font-semibold">
                {data?.otherSkill.map((skill, index) => (
                  <span
