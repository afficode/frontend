...
export const removeNullObjectsValues = (productDetails) => {
	...
	// filter the array
	const filteredEntries = entries.filter(([_key, value]) => value !== null && value !== '');
...
export const manipulateFilterForm = (values, category_id) => {
	...
	let filteredValue = removeNullObjectsValues(values);
...
export const extractAdIdFromSlug = (slug) => {
...
export const getCategoryName = (categoryId) => {
	const name = categoriesAndSubCategories.find((category) => category.id === categoryId)?.name || 'vehicle';
	return name
		.toLowerCase()
		.replace(/[^a-z0-9]+/g, '-')
		.replace(/^-|-$/g, '');
...
export const getCategoryId = (categorySlug) => {
	if (!Number.isNaN(parseInt(categorySlug))) {
		return categorySlug;
	}
	const category = categoriesAndSubCategories.find((category) => categorySlug === getCategoryName(category.id));
	return category ? category.id : categoriesAndSubCategories[0].id;
};
