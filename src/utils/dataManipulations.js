...
export const convertKeyToName = (ad) => {
  const keys = ["year", "color", "ad_condition", "property_type", "use", "size", "date_available", "facilities", "company_name", "service_type", "years", "mode_of_charge", "available_for_travel", "brand", "type", "material", "bulk_price", "gender", "age", "display_size", "style", "formulation", "scent_type", "platform", "format", "game_genre", "seller_type", "vehicle_specification", "make", "model", "vehicle_body", "vehicle_features", "transmission", "millage", "number_of_seat", "number_of_door", "fuel_type", "engine_size"]
  var overviews = [];
  if(ad === null || undefined) {
    ...
    keys.forEach(key => {
      if(key in ad) {
        if(key.includes("_")) {
          const name = key.split("_").map(val => val[0].toUpperCase() + val.substring(1)).join(" ");
          overviews.push({
            name,
            param: key,
