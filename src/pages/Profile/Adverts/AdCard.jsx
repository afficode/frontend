...
const AdCard = ({ title, images, active, price, views, adId, chats, available, feature, grab_activity }) => {
	...
	const renewAdvert = () => {
		...
		mutate(formData, {
			...
			onError: () => {
				notify('Error renewing Ad. If this error persist, please contact Admin with the contact us form.', 'error');
			},
		});
	};
...
'
			...
			<div className={isExpired ? 'absolute inset-0 pointer-events-none z-10 bg-[#D9D9D9B0] flex items-center justify-center' : 'hidden'}>
				<div className='flex items-center gap-4 w-full px-2'>
					...
					<Button
						variant={'primary'}
						size={'full'}
						className={'text-white !bg-gray-600 rounded-md pointer-events-auto !px-4 !w-full '}
						onClick={(e) => {
							e.stopPropagation();
							e.preventDefault();
