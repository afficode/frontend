			...
			<div className='mt-16 mb-4'>
				...
				<div className='max-w-[30rem] mx-auto mt-6 lg:mt-[-3rem]'>
					<Link to={Approutes.product.initial}>
						<button className='w-full text-lg capitalize border-none btn text-white hover:bg-secondary hover:border-black/50 hover:border-solid bg-secondary rounded-3xl sm:text-xl '>View Ads</button>
					</Link>
				</div>
			</div>
		</section>
...
const Card = ({ title, image, description, action, link }) => {
	return (
		<div className='w-full sm:max-w-[18rem] h-[23rem] flex flex-col items-center hero-card text-center justify-between rounded-md px-4 py-6 bg-white shadow-md cursor-pointer hover:scale-105 hover:shadow-lg transition-all '>
			...
			<Link to={link} className='mt-4 text-lg capitalize border-none btn rounded-3xl hover:border-black/50 hover:border-solid hover:bg-secondary/90 bg-secondary px-8 text-white'>
				{action}
			</Link>
		</div>
	);
...
const cardData = [
	...
	},
	{
		...
		action: 'Search Now',
		link: `${Approutes.product.category}/${getCategoryName(52)}`,
	},
	{
		title: 'Property',
		image: Property,
