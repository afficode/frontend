	...
	return (
		<>
			{product.map((ad, index) => (
				<div
					onClick={() => navigate(`/product/${slugGeneratorForAdIdWithName(ad.title, ad.id)}`)}
					key={index}
				...
				>
					<div
						onClick={(e) => {
							...
							const isArrow = target.closest('button[data-testid="carousel-right-control"], button[data-testid="carousel-left-control"]');
					...
					>
						{ad.images.length > 0 ? (
							<Carousel className='h-[200px] md:h-[230px] rounded-none'>
								{ad.images.map((img, index) => (
									<img src={img.path} alt={img.filename} key={index * 3} className='w-full h-full  rounded-b-none ' />
								))}
							</Carousel>
						) : (
						...
						{((isLogin && ad?.owner !== user?.id) || !isLogin) && <SaveProduct ads_id={ad.id} className='absolute w-10 h-12 p-1 px-2 bg-gray-200 rounded shadow-2xl top-4 right-4 hover:bg-white ' />}
					...
					<div className='w-full p-2 tracking-tighter tooltip tooltip-secondary line-clamp-1 '>
						<p className='flex items-start justify-start h-6 overflow-hidden text-xl font-semibold uppercase lg:h-8 '>{ad.title.toString().trimEnd().trim()} </p>
