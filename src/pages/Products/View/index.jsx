...
import { numberWithCommas, convertKeyToName, extractAdIdFromSlug, slugGeneratorForAdIdWithName } from '../../../utils/index.js';
...
const index = () => {
	...
	useEffect(() => {
		if (result?.data) {
			setItems(() => [{ name: 'Home', link: Approutes.home }, { name: 'Products', link: Approutes.product.initial }, { name: result?.data?.title }]);
		}
		...
		<section className='w-full px-4 py-2 lg:py-4 lg:px-8'>
			{result?.data?.paid === 1 && result?.data?.active === '2' ? (
				...
				<div className='w-[90%] lg:w-[70%] my-3 mx-auto'>
					...
					>
						...
						<div className=''>
							...
							<div className='w-full font-bold my-2 bg-secondary text-white p-2'>Please ensure the Ad title matches exactly what you have in your Ad. This will facilitate the response to your request.</div>
						</div>
					</Alert>
				</div>
			) : (
				...
				user.id === result?.data.owner && (
					<div className='w-[90%] lg:w-[70%] my-3 mx-auto'>
						<Alert additionalContent={<ContactAdmin ads_id={id} />} color='warning' icon={HiInformationCircle}>
							<div>
								<span className='font-medium text-red-600'>
									Ad Blocked: <span className='underline'> Change a few things up and try submitting again.</span>
								</span>{' '}
							</div>
						</Alert>
					</div>
				...
				<main className='w-full flex flex-col'>
					<div className='w-full my-2 '>
						...
						<div className='flex items-center justify-between'>
							...
							{result?.data?.active === '0' && result?.data.paid === 1 ? <span className={'px-4 py-1 rounded-lg text-white bg-red-700  '}>Blocked</span> : null}
						</div>
					</div>{' '}
							...
							{result.data?.images.length > 0 ? (
								<Carousel className='h-full max-lg:h-[300px] md:h-[470px] rounded-none'>
									{result.data?.images.map((img, index) => (
										<img src={img.path} alt={img.filename} key={index * 3} className='rounded-t-sm rounded-b-none ' />
									))}
								</Carousel>
							) : (
			...
			<section className='flex flex-col p-2 my-2 bg-gray-200 xl:p-6 xl:my-4'>
				<div className='flex flex-col items-start justify-start w-full gap-2 tracking-tighter lg:tracking-normal line-clamp-1'>
					...
					<p className='bg-white p-4 min-h-[100px] text-justify text-lg border-t-4 border-t-primary whitespace-pre-line w-full'>{result?.data?.description}</p>
				</div>
			...
			<section className='flex flex-col  h-full w-full gap-2 md:flex-row md:items-stretch md:gap-8 line-clamp-1'>
				<main className='w-full h-full  md:w-[60%] xl:w-[70%] flex flex-col justify-between lg:py-4 border-2 border-transparent'>
					...
					<div className='w-full mb-2 pl-2'>
						...
						<div className='flex items-center justify-between'>
							<p className='w-full'>
								<Link to={`/product/?q=&state_id=${result?.data?.state_id}&lga_id=${result?.data?.lga_id}`} className='text-primary hover:underline'>
									{result?.data?.location.split(',')[0]}
								</Link>{' '}
								|{' '}
								<Link to={`/product/?q=&state_id=${result?.data?.state_id}`} className='text-primary hover:underline'>
					...
					<div className='relative w-full h-full mx-auto mt-1  rounded-none '>
						{result?.data?.images.length > 0 ? (
							...
							<img src={noimage} alt='no image' className='rounded-sm  h-[250px] md:min-h-[650px] w-full object-cover' />
						)}
				...
				<aside className='w-full h-full md:w-[40%] xl:w-[30%] border-2 border-gray-400 max-lg:p-2 lg:px-4 lg:pt-4 flex flex-col'>
						...
						<div className='w-full text-lg tracking-tighter lg:text-xl'>
							...
							{result?.data?.contact_type.includes('phone') && (
								<div className='flex items-center justify-between'>
									<p className='my-2 text-sm  text-start'>
										<span className=' font-bold'>{revealNumber ? result?.data?.number : `${result?.data?.number.substring(0, 3)}XXXXXXXX`}</span>
									</p>
							...
							{result?.data?.contact_type.includes('email') && (
								<div className='flex items-center justify-between w-full'>
									<p className={`my-2 w-full overflow-x-scroll  text-start ${revealEmail ? 'tooltip tooltip-primary' : ''}`} data-tip={revealEmail ? result?.data?.email : ''}>
										<span className='pr-1 text-sm font-bold'>{revealEmail && result?.data?.email !== null ? result?.data?.email : `${result?.data?.email.substring(0, 3)}XXXXXXXX`}</span>
									</p>
					...
					<ChatForm ad={result?.data} ad_id={result?.data?.id} feature={result?.data?.feature} owner={result?.data?.owner} active={result?.data?.active} />
				</aside>
			</section>
