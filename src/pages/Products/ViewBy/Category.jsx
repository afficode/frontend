...
import { Approutes, COMPANY_NAME, frontendLink, queryStrings, WELCOME_BOONFU_IMAGE } from '../../../constants';
...
import { getCategoryId, getPreviousSearchParams, getSearchParamsObject, ScrollToTop } from '../../../utils';
...
const Category = () => {
	...
	useEffect(() => {
		if (categories) {
			...
			setItems(() => [{ name: 'Home', link: Approutes.home }, { name: 'Products', link: Approutes.product.initial }, { name: 'Categories', link: Approutes.product.category }, { name }]);
		...
		<section className='flex gap-x-2'>
			...
			<main className='p-2 w-full md:w-[70%] lg:w-[75%]'>
				<div className='w-full md:hidden '>
					<Drawer icon={<VscMenu className='text-primary text-xl ' />} items={<SideBar displayCategories={displayCategories} currentCategoryId={Number(currentCategoryId)} />} />
				</div>
				<div className='w-full my-4'>{!isLoading && <Breadcrumb items={items} className={'text-md breadcrumbs text-primary'} />}</div>
