...
const Category = () => {
  ...
  const [searchParams, setSearchParams] = useState({ category: categoryId });
  const { data, isLoading, error } = useProduct(searchParams);
          ...
          {isLoading ||
            (data && data?.ads.length > 0 && (
              <div className="join mx-auto mt-4 bg-primary text-white">
                <button
                  onClick={() => {
                    setSearchParams((prev) => ({ ...prev, page: data?.prev }));
                ...
                <button
                  ...
                  onClick={() => {
                    setSearchParams((prev) => ({ ...prev, page: data?.next }));
                    // window.location.reload();
                  }}
                  className={`${
