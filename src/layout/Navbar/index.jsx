...
import { getCategoryName, toSelectOptions } from '../../utils';
                ...
                <div className="w-full pt-3 ">
                    ...
                    <div className="max-w-[1380px] mx-auto px-2 flex items-center justify-between ">
						...
						<div className='flex items-center gap-2 lg:gap-3'>
							{isLogin && (
								...
								<>
									<Link to={Approutes.profile.notifications}>
										<div className='flex flex-col justify-center items-center p-2 bg-white rounded-full relative' title='My Notifications'>
							...
							<Link to={Approutes.profile.saved}>
								<div className='relative flex flex-col items-center text-white cursor-pointer max-md:hidden' title='Saved items'>
									<GoBookmark size={25} />
									<span className='text-xs sm:text-sm'>Saved</span>
									{isLogin && saves?.saves.length > 0 && (
										<span className='absolute top-[-7px] rounded-full px-1 bg-secondary/90 right-0 font-semibold text-sm text-black'>{saves?.saves.length}</span>
									)}
								</div>
							</Link>
							{/* post ad dropdown */}
							<div className='dropdown '>
								<div tabIndex={0} className='flex flex-col items-center text-white cursor-pointer ' title='Post an ad'>
									<HiOutlineSpeakerphone size={25} />
									<span className='text-xs sm:text-sm whitespace-nowrap'>Post ad</span>
								</div>
								<ul tabIndex={0} className={'dropdown-content transform -translate-x-2/3 min-h-fit w-fit  z-[10] px-4 py-6 bg-white shadow-md rounded-md '}>
											...
											<ul className='flex flex-col menu max-h-full w-full z-[10] py-4 '>
												{filteredCategories?.allCat?.slice(0, 14)?.map((category) => (
													<Link to={`${Approutes.postAd}/${category.id}`} key={category.id}>
														<li className='text-lg capitalize max-sm:text-base lg:pr-12 hover:underline whitespace-nowrap'>{category.name}</li>
													</Link>
												))}
											</ul>
										</>
							...
							{!isLogin ? (
								<Link to={Approutes.auth.initial}>
									<div className='flex flex-col items-center text-white cursor-pointer' title='Click to sign-in or register'>
										<CgProfile size={25} />
								...
								<>
									<Link to={Approutes.profile.messages}>
										<div className='relative flex flex-col items-center  text-white cursor-pointer max-md:hidden' title='My messages'>
											{' '}
											{isLogin && unread > 0 && <span className='absolute top-[-5px] rounded-full px-1 bg-secondary/90 right-[7px] font-semibold text-sm text-black'>{unread}</span>}
											<BiEnvelope size={25} />
									...
									<div className='dropdown dropdown-hover'>
										<Link to={Approutes.profile.initial}>
											<div tabIndex={0} className='flex flex-col max-sm:hidden items-center text-white cursor-pointer' title='My profile'>
												<CgProfile size={25} />
												<span className='text-xs sm:text-sm whitespace-nowrap'>{user?.firstname}</span>
											</div>
										</Link>
										<ul tabIndex={0} className={'dropdown-content transform -translate-x-[50%] sm:-translate-x-[84%] min-h-fit w-[15rem]  z-[10] p-4 bg-white shadow-md rounded-2xl'}>
							...
							{!isLogin ? (
								<div className='dropdown dropdown-end'>
									<button className='flex flex-col gap-0 px-4 py-0 capitalize bg-white border-none max-sm:text-xs lg:hidden btn btn-sm text-primary hover:bg-white'>Categories</button>
									<ul tabIndex={0} className={'dropdown-content min-h-fit w-fit z-[10] px-4 py-6 bg-white shadow-md rounded-md'}>
										<h3 className='font-semibold max-lg:text-xl whitespace-nowrap'>Categories</h3>
										<ul className='flex flex-col menu max-h-full w-full z-[10] py-4 '>
											{filteredCategories?.allCat?.map((category) => (
												<NavLink to={`${Approutes.product.category}/${getCategoryName(category.id)}`} key={category.id}>
													<li className='text-lg capitalize max-sm:text-base lg:pr-12 hover:underline whitespace-nowrap'>{category.name}</li>
												</NavLink>
											))}
										</ul>
					...
					<div className='mt-2 border-y-2 border-y-white '>
						<div className='max-w-[1380px] mx-auto px-2 relative flex items-center justify-between w-full  whitespace-nowrap'>
							...
							<div className='dropdown '>
								<button className='flex flex-col gap-0 px-5 mr-16 text-sm capitalize bg-white border-none max-lg:hidden btn btn-sm hover:bg-white text-primary cat-btn'>Browse by Categories</button>

								<ul tabIndex={0} className={'dropdown-content transform -translate-x-[1%] min-h-fit w-fit  z-[10] px-4 py-6 bg-white shadow-md rounded-md'}>
							...
							<ul className='flex items-center justify-between w-full'>
								<li className='dropdown dropdown-hover'>
									<a href={`${Approutes.product.category}/${getCategoryName(50)}`} tabIndex={0} className={listStyles}>
										CARS & VEHICLES
									</a>
									<a href={`${Approutes.product.category}/${getCategoryName(50)}`} className={mobileListStyles}>
										<FaCarSide size={25} />
									</a>
									{filteredCategories?.vehicleCat && <CategoryDropdown category={'CARS & VEHICLES'} subCategories={filteredCategories?.vehicleCat} />}
								</li>

								<span className='border border-r-4 border-white h-[2rem]' />

								<li className='dropdown dropdown-hover'>
									<a href={`${Approutes.product.category}/${getCategoryName(51)}`} tabIndex={0} className={listStyles}>
										PROPERTY
									</a>
									<a href={`${Approutes.product.category}/${getCategoryName(51)}`} className={mobileListStyles}>
										<FaBuilding size={25} />
									</a>
									{filteredCategories?.propertyCat && <CategoryDropdown category={'PROPERTY'} subCategories={filteredCategories?.propertyCat} />}
								</li>

								<span className='border border-r-4 border-white h-[2rem]' />

								<li className='dropdown dropdown-hover '>
									<a href={`${Approutes.product.category}/${getCategoryName(52)}`} tabIndex={0} className={listStyles}>
										SERVICES
									</a>
									<a href={`${Approutes.product.category}/${getCategoryName(52)}`} className={mobileListStyles}>
										<MdMiscellaneousServices size={25} />
									</a>
									{filteredCategories?.servicesCat && <CategoryDropdown category={'SERVICES'} subCategories={filteredCategories?.servicesCat} />}
								</li>

								<span className='border border-r-4 border-white h-[2rem]' />

								<li className='dropdown dropdown-hover '>
									<a href={`${Approutes.product.category}/${getCategoryName(53)}`} tabIndex={0} className={listStyles}>
										AGRICULTURE
									</a>
									<a href={`${Approutes.product.category}/${getCategoryName(53)}`} className={mobileListStyles}>
										<FaRegHandshake size={25} />
									</a>
									{filteredCategories?.agricCat && <CategoryDropdown category={'AGRICULTURE'} subCategories={filteredCategories?.agricCat} />}
								</li>

								<span className='border border-r-4 border-white h-[2rem]' />

								<li className='dropdown dropdown-hover mr-4'>
									<a href={`${Approutes.product.category}/${getCategoryName(54)}`} tabIndex={0} className={listStyles}>
										ELECTRONICS
									</a>
									<a href={`${Approutes.product.category}/${getCategoryName(54)}`} className={mobileListStyles}>
										<VscGitPullRequestGoToChanges size={25} />
									</a>
									{filteredCategories?.electCat && <CategoryDropdown category={'ELECTRONICS'} subCategories={filteredCategories?.electCat} />}
								</li>
							</ul>
						</div>
					</div>
...
const listStyles = 'hidden md:block uppercase flex-grow-0 flex-shrink-0 cursor-pointer text-white text-sm md:text-base font-medium py-2 px-4  hover:text-secondary transition-colors ';
