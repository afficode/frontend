...
const Dropdown = ({ category, subCategories }) => {
  return (
    <div
      tabIndex={0}
      className={`${
        category.includes("CARS") ? "transform -translate-x-1/4" : ""
    ...
    >
      <h5 className="font-semibold max-md:whitespace-normal">
        Browse sub-category in: {category}
      </h5>
      <ul className="flex flex-col gap-[0.1rem] menu max-h-full w-full md:min-w-[40rem] py-4 ">
        {subCategories.map((subCategory) => (
          <a
            href={`${Approutes.product.category}/${btoa(subCategory.id)}`}
            key={subCategory.id}
          >
