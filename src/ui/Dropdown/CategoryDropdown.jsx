		...
		>
			...
			<ul className='grid grid-cols-1 md:grid-cols-2 gap-[0.1rem] menu max-h-full w-full md:w-[35rem]  py-4 '>
				{subCategories.map((subCategory) => (
					<a href={`${Approutes.product.category}/${getCategoryName(subCategory.id)}`} key={subCategory.id}>
						<li className='text-lg capitalize whitespace-normal max-sm:text-base lg:pr-12 hover:underline'>{subCategory.name}</li>
					</a>
				))}
			</ul>
		</div>
